...
The macOS configuration assumes lksd will be set up for the current user.

Configuration
---------------------------------

Running lksd as a daemon does not require any manual configuration. You may
set the `rpcauth` setting in the `lks.conf` configuration file to override
...
PID file:            `/var/run/lksd/lksd.pid` (OpenRC and Upstart) or `/run/lksd/lksd.pid` (systemd)
...
systemd. Directories are given a permission of 710, giving the lkscore user and group
access to files under it _if_ the files themselves give permission to the
lkscore user and group to do so (e.g. when `-sysperms` is specified). This does not allow
for the listing of files under the directory.

NOTE: It is not currently possible to override `datadir` in
`/etc/lks/lks.conf` with the current systemd, OpenRC, and Upstart init
files out-of-the-box. This is because the command line options specified in the
init files take precedence over the configurations in
`/etc/lks/lks.conf`. However, some init systems have their own
