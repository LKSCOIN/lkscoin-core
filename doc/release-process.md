...
* Update translations, see [translation_process.md](https://github.com/LKSCOIN/lkscoin-core/blob/master/doc/translation_process.md#synchronising-translations).

* Update manpages, see [gen-manpages.sh](https://github.com/LKSCOIN/lkscoin-core/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
Check out the source code in the following directory hierarchy.
	...
	git clone https://github.com/LKSCOIN/lkscoin-core-detached-sigs.git
	git clone https://github.com/devrandom/gitian-builder.git
	git clone https://github.com/LKSCOIN/lkscoin-core.git
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in lks, then:
...
### Build and sign Lksc Core for Linux, Windows, and macOS:
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`lks-${VERSION}-osx-unsigned.dmg`, `lks-${VERSION}-osx64.tar.gz`)
...
Codesigner only: Sign the macOS binary:
    ...
    transfer lkscore-osx-unsigned.tar.gz to macOS for signing
...
- Detached signatures will then be committed to the [lks-detached-sigs](https://github.com/LKSCOIN/lkscoin-core-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
lks-${VERSION}-riscv64-linux-gnu.tar.gz
...
space *do not upload these to the lksfoundation.org server*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the lksfoundation.org server

- Update lksfoundation.org

- Announce the release:

  - Release on Lks forum: https://www.lksfoundation.org/forum/topic/official-announcements.54/

  - Optionally Discord, twitter, reddit /r/Lkspay, ... but this will usually sort out itself

  - Notify flare so that he can start building [the PPAs](https://launchpad.net/~lksfoundation.org/+archive/ubuntu/lks)

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/LKSCOIN/lkscoin-core/releases/new) with a link to the archived release notes.
...
- For `mainnet` -> Take the size of the Lksc Core data directory, excluding `/regtest` and `/testnet3` directories.
