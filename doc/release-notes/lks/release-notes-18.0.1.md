...
been added to support Lks Platform. This update introduces versioning to
...
Governance
----------
Several improvements have been made to Lks’s DAO governance system.
The governance proposal fee has been reduced from 5 Lks to 1 Lks following
...
`salvage` command in the `lks-wallet` tool.
...
- The `testnet` field in `lks-cli -getinfo` has been renamed to `chain` and
...
deprecated. `size` will only be returned if `lksd` is started with
`-deprecatedrpc=size`.
...
---------------------------
This release introduces over 1000 updates from Bitcoin v0.18/v0.19/v0.20 as well as numerous updates from Bitcoin v0.21 and more recent versions. This includes multi-wallet support in the GUI, support for partially signed transactions (PSBT), Tor version 3 support, and a number of other updates that will benefit Lks users. Bitcoin changes that do not align with Lks’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes – v0.18, v0.19, v0.20.
