...
Currently two Docker containers exist, `ci` defines how Lks's GitLab CI container is built and the `dev` builds on top of the `ci` to provide a containerized development environment that is as close as possible to CI for contributors!
...
to run terminal commands from inside the terminal and build Lksc Core.
