...
# Copyright (c) 2014-2022 The Lksc Core developers
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--lksd-arg", dest="lksd_extra_args", default=[], action="append",
    ...
    def setup(self):
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/lksd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/lks-cli' + config["environment"]["EXEEXT"])
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in lks.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if lksd has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if lks-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("lks-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if lks-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("lks-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether lks-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether lks-wallet was compiled."""
...
class LksTestFramework(BitcoinTestFramework):
    ...
    def set_lks_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
        ...
        # This is nRequestTimeout in lks-q-recovery thread
